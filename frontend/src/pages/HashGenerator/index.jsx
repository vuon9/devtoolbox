...
const METHODS = [
  'All', 'MD5', 'SHA-1', 'SHA-224', 'SHA-256', 'SHA-384', 'SHA-512',
  'SHA-3 (Keccak)', 'BLAKE2b', 'BLAKE3', 'RIPEMD-160', 'bcrypt', 'scrypt',
  'Argon2', 'HMAC', 'CRC32', 'Adler-32', 'MurmurHash3', 'xxHash', 'FNV-1a',
];
...
export default function HashGenerator() {
  ...
  const performHash = useCallback(async (text, meth) => {
    if (!text && meth !== 'All') {
      setOutput('');
      setError('');
      return;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '24px', overflow: 'hidden', backgroundColor: '#09090b' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
        ...
        {isHmac && (
          <input type="text" value={hmacKey} onChange={(e) => setHmacKey(e.target.value)} placeholder="HMAC Key"
            style={{ height: '36px', padding: '0 12px', fontSize: '13px', borderRadius: '6px', backgroundColor: 'var(--card)', border: '1px solid var(--border)', color: 'var(--foreground)', outline: 'none', width: '200px' }}
          />
        )}
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        <ToolSplitPane isVertical={isVertical}>
          <ToolPane label="Input" value={input} onChange={(val) => setInput(val)} placeholder="Enter text to hash..." indicator="Source" indicatorColor="green" highlightOn={highlightOn} />
          {isAll ? (
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, border: '1px solid var(--border)', borderRadius: '8px', backgroundColor: 'var(--background)', overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderBottom: '1px solid var(--border)', backgroundColor: 'var(--card)' }}>
                <label style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--muted-foreground)' }}>
                  Output
                </label>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em',
            ...
            <ToolPane label="Output" value={output} readOnly placeholder="Hash result will appear here..." indicator="Result" indicatorColor="blue" error={!!error} highlightOn={highlightOn} />
          )}
        </ToolSplitPane>
      </div>
    </div>
