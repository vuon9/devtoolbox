...
export default function RegExpTester() {
  ...
  const [isVertical, setIsVertical] = useState(() => localStorage.getItem('regex-layout') === 'vertical');
      ...
      <ToolControls className="mb-6 justify-between items-end gap-6">
        <div className="flex-1 max-w-xl flex items-end gap-4">
          <div className="flex-1">
            <Label htmlFor="regex-input" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70 mb-1.5 ml-1">Pattern</Label>
            <div className="relative group">
              <span className="absolute left-3 top-2.5 h-4 w-4 text-primary font-bold opacity-40 font-mono">/</span>
              <Input
                id="regex-input"
                value={regex}
              ...
              />
              <span className="absolute right-3 top-2.5 h-4 w-4 text-primary font-bold opacity-40 font-mono">/</span>
            </div>
          </div>
          <div className="w-24">
            <Label htmlFor="flags-input" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70 mb-1.5 ml-1">Flags</Label>
            <Input
              id="flags-input"
              value={flags}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isVertical ? <Columns className="h-5 w-5 rotate-90" /> : <Columns className="h-5 w-5" />}
          </Button>
        </div>
      </ToolControls>
          ...
          <div className="flex flex-col h-full space-y-4 overflow-hidden">
             <div className="p-6 rounded-lg bg-muted/20 border border-border/40 flex-1 flex flex-col min-h-0 shadow-sm">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 font-mono">
                  {matches.map((match, i) => (
                    <div key={i} className="p-3 rounded border border-border/40 bg-background/50 group hover:border-primary/30 transition-all shadow-sm">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-primary/40">Match {i + 1}</span>
                        <span className="text-[10px] font-medium text-muted-foreground/30">Index: {match.index}</span>
                      </div>
                      <div className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{match[0]}</div>
                      {match.length > 1 && (
                        <div className="mt-3 grid grid-cols-1 gap-1 border-t border-border/20 pt-2">
                          {Array.from(match).slice(1).map((group, j) => (
                            <div key={j} className="text-[11px] flex gap-2 overflow-hidden">
                              ...
                              <span className="text-foreground/80 truncate font-medium">{group || 'null'}</span>
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  {matches.length === 0 && !error && (
                    <div className="h-full flex flex-col items-center justify-center text-muted-foreground opacity-30 gap-4">
                       ...
                       <div className="text-xs font-bold uppercase tracking-widest">No matches found</div>
                    </div>
                  )}
                </div>
             </div>
