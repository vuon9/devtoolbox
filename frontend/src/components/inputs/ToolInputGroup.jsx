...
export default function ToolInputGroup({
  ...
  if (editable) {
    ...
    if (activeTab === 0) {
      ...
      const displayText =
        validation.isValid && validation.data ? objectToKeyValueString(validation.data) : '';

      content = <textarea value={displayText} readOnly style={textareaStyle} />;
    ...
    if (activeTab === 0) {
      ...
      content = <textarea value={jsonText} readOnly style={textareaStyle} />;
      copyText = jsonText;
    } else {
      // Claims tab - key: value format
      const claimsText = data ? objectToKeyValueString(data) : '';
      content = <textarea value={claimsText} readOnly style={textareaStyle} />;
      copyText = claimsText;
    }
  }

  // Validation error for editable JSON
  const validationError = editable && activeTab === 0 && value ? validateJson(value).error : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showCopyButton && <ToolCopyButton text={copyText} disabled={!copyText} />}
      </div>

      {/* Tab bar */}
      <ToolTabBar tabs={tabs} activeTab={activeTab} onChange={onTabChange} />
